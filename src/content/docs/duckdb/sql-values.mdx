...
# Waddler SQL values
<Prerequisites>
- Quick start with Waddler - [read here](/docs/duckdb/get-started/duckdb-neo-new)
...
Waddler provides a convinient [`sql.default`](/docs/duckdb/sql-default) for SQL `DEFAULT` values:
```ts
...
Complete example with [`sql.identifier`](/docs/duckdb/sql-identifier) and [`sql.default`](/docs/duckdb/sql-default):
<Section>
```ts
