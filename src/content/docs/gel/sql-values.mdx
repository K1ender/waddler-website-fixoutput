...
Waddler provides a convinient [`sql.default`](/docs/gel/sql-default) for SQL `DEFAULT` values:
```ts
...
Complete example with [`sql.identifier`](/docs/gel//sql-identifier) and [`sql.default`](/docs/gel/sql-default):
<Section>
```ts
