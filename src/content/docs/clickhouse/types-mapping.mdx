...
When you pass parameters using `${...}`, `sql.param` or `sql.values`,  each one is replaced in the query by a placeholder of the form `{<parameter name>:<database type>}` 
and a corresponding entry `{<parameter name>:<parameter value>}` will be added to the parameters object.
...
```
In this example, the `${...}` syntax is used to specify a parameter in the query, 
and the `{param1:Int32}` notation represents that parameter in the parametrized query. 

Specifying `Int32` as the `database type` indicates that the parameter should be sent to the database as an `Int32`.
...
|:-| :--------                            | :--------                            | :--------    
|  | `integers (within 32-bit range)`     |`${-2147483648}` or `${3}`            | `Int32`    
|  | `53-bit integers (JS safe integers)` |`${1754055760745}`                    | `Int64`    
|  | `bigint`                             |`${1754055760745n}` or `${BigInt(3)}` | `Int64`      
|  | `one dimensional string array`       |`${['/', '/watch']}`                  | `Array(String)`    
